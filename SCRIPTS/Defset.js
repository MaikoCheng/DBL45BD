//This script runs when the default data button is pressed.
...
//When default data button is pressed
 ...
 d3.csv(dataURL, function(data) {
    ...
    nodes
      .on('mouseover', function (d) {
        ...
        //Highlight the links corresponding to the highlighted node
        links
          .style('stroke', function (link) {if (d != undefined){ return link.fromEmail === d.name || link.toEmail === d.name ? color(d.jobtitle) : '#b8b8b8';}})
          .style('stroke-opacity', function (link) {if (d != undefined){ return link.fromEmail === d.name || link.toEmail === d.name ? 5 : .2;}})
          .style('stroke-width', function (link) {if (d != undefined){ return link.fromEmail === d.name || link.toEmail === d.name ? 4 : 1;}})

        //Highlight the label: font-size increases for the selected node, other nodes get smaller font-size
        labels
          .style("font-size", function(label){if (label != undefined){ return label.name === d.name ? 200 : 20 }} )

        //Highlight the cells in the adjacency matrix corresponding to the highlighted node
        cells
    ...
    //Circles job titles legend adj matrix 
    ...
    //Labels job titles legend adj matrix 
    ...
    svg3.append("g")
        ...
        .call( d3.brush()                         // Add the brush feature using the d3.brush function    
          .extent([[-20,-20],[width2, width2]])   // initialise the brush area: start at -20,-20 and finishes at width2,height2: it means the whole graph area with some extra space at the top and left
          .on("brush", doOnBrush)
          .on("start", mouseout))
...
//Chechboxes for job title filter
